...
const Modal = ({ setIsOpen, allData }) => {
    ...
    const [updateSubmissionById] = useMutation(UPDATE_SUBMISSION_BY_ID, {
        refetchQueries: [{ query: ALL_SUBMISSION }],
    })
    ...
    const handleOnSubmit = useCallback(
        async (e) => {
            e.preventDefault()
            try {
                await updateSubmissionById({
                    variables: { id: allData?._id, record: submission },
                })
                setIsOpen(false)
            } catch {
                console.log('error')
    ...
    return (
        <Fragment>
            <div className="w-full h-full absolute bg-gray-200 bg-opacity-60 top-0 left-0 flex items-center justify-center">
                <form
                    className="w-1/2  bg-white flex flex-col shadow-lg rounded-xl pl-5 pt-2"
                    onSubmit={handleOnSubmit}
                >
                    <div className="w-full grow flex flex-col space-y-3 mb-2">
                        <div className="w-full h-1/4 flex p-3 flex-wrap space-y-1">
                            <h1 className="w-3/4 text-lg font-bold font-serif -mb-1">
                                {allData?.form_id?.title ?? 'No title'}
                            </h1>
                            ...
                            <h1 className="mr-2">
                                {allData?.submitted_by?.fullname ?? 'No data'}
                            </h1>
                        ...
                        <div className="w-full grow p-3 flex ">
                            ...
                            <div className="w-1/2 pl-5 h-full">
                                ...
                                <select
                                    className="w-11/12 border border-gray-300 p-2 rounded-lg focus:outline-none"
                                    onChange={handleOnChange}
                                    id="status"
                                >
                                    <option className="">
                    ...
                    <div className="w-full h-14 flex space-x-5 justify-center">
                        <div>
                            <button
                                type="submit"
                                className="bg-black rounded-md text-white w-36 py-2 mr-5 text-medium font-semibold"
                            >
                                Save
                            </button>
                            <button
                                className="bg-slate-200 rounded-md w-36 py-2 text-medium font-semibold"
